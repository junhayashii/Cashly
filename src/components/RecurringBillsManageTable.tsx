...
interface RecurringBillsManageTableProps {
  ...
  onSelectRecurringBill?: (bill: RecurringBill | null) => void;
}
...
const dueToneDecor: Record<DueTone, string> = {
  muted: "bg-muted/40 text-muted-foreground",
  warning:
    "bg-amber-500/15 text-amber-600 dark:bg-amber-500/10 dark:text-amber-300",
  destructive:
    "bg-red-500/15 text-red-600 dark:bg-red-500/10 dark:text-red-300",
  success:
          ...
          {initialLoading ? (
            ...
            <div className="space-y-2">
              ...
              {filteredBills.map(({ bill, nextDueDate, monthlyStatus }) => {
                ...
                const handleSelectRecurringBill = () => {
                  if (!onSelectRecurringBill) return;
                  onSelectRecurringBill(isSelected ? null : bill);
                };

                return (
                  <div
                    key={bill.id}
                    className={`group relative flex flex-col gap-3 rounded-xl border border-border/30 bg-gradient-to-br from-background/70 via-background/90 to-background/70 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary/10 hover:shadow-lg ${
                      onSelectRecurringBill
                    ...
                    onClick={handleSelectRecurringBill}
                    onKeyDown={(event) => {
                      if (
                        !onSelectRecurringBill ||
                      ...
                      handleSelectRecurringBill();
                    }}
                    role={onSelectRecurringBill ? "button" : undefined}
                    tabIndex={onSelectRecurringBill ? 0 : undefined}
                    aria-pressed={onSelectRecurringBill ? isSelected : undefined}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-1 text-[10px] font-medium ${dueToneDecor[dueDescriptor.tone]}`}
                        >
                          {dueDescriptor.label}
                        </span>
                      </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {!isCredit && (
                            <Button
                              ...
                              className="h-8 gap-1.5 border-border/60 px-3 text-xs font-medium hover:bg-accent/40"
                            >
                              Pay
                            </Button>
                          )}
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                ...
                                className="h-7 w-7 text-muted-foreground hover:bg-accent/40"
